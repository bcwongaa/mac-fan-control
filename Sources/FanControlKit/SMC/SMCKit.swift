...
final class SMCKit {
    ...
    func readTemperature(key: String) throws -> Double {
        ...
        return try SMCDataCodec.decodeTemperature(bytes, dataType: dataType)
    }

    func readFanRPM(key: String) throws -> Double {
        let (dataType, bytes) = try read(key)
        return try SMCDataCodec.decodeFanRPM(bytes, dataType: dataType)
    }

    /// Read a ui8 value (e.g. F0Md, Ftst)
    func readUInt8(key: String) throws -> UInt8 {
        let (dataType, bytes) = try read(key)
        return try SMCDataCodec.decodeUInt8(bytes, dataType: dataType)
    }

    // MARK: - Debug Dump

    /// Resolve candidate keys once so polling only touches sensors present on this Mac.
    func resolveReadableKeys(_ candidates: [String]) -> [String] {
        candidates.filter { (try? read($0)) != nil }
    }

    /// Read-only dump of fan and known Apple Silicon temperature keys.
    func dumpAllKeys() -> String {
        ...
        let keysToCheck = fanKeys + TempKey.allKnownDieCandidates + [
            "TC0P", "TC0D", "TG0P", "TG0D",
        ]
        ...
        for key in keysToCheck {
            do {
                ...
                var decoded = ""
                if typ == "flt", let value = try? SMCDataCodec.decodeTemperature(bytes, dataType: dataType) {
                    decoded = String(format: "%.2f", value)
                }
                else if typ == "sp78", let value = try? SMCDataCodec.decodeTemperature(bytes, dataType: dataType) {
                    decoded = String(format: "%.2f °C", value)
                }
                else if typ == "fpe2", let value = try? SMCDataCodec.decodeFanRPM(bytes, dataType: dataType) {
                    decoded = String(format: "%.2f RPM", value)
                }
